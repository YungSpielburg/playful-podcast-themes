  ...
  return (
    <section id="portfolio" className="py-20 md:py-32 relative overflow-hidden">
      <div className="absolute inset-0 -z-10">
        <div className="absolute top-40 left-20 w-64 h-64 bg-teal/5 rounded-full filter blur-3xl"></div>
        <div className="absolute bottom-20 right-20 w-96 h-96 bg-accent-purple/5 rounded-full filter blur-3xl"></div>
      </div>
      ...
      <div className="section-container">
        <div className="text-center max-w-3xl mx-auto">
          ...
          >
            Trusted by the World's <span className="text-teal">Top Podcasts</span>
          </motion.h2>
          <motion.p 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-16">
          {clientLogos.map((client, index) => (
            <motion.div
              ...
              className={`rounded-xl overflow-hidden ${client.highlight ? 'glass' : 'bg-white/50 backdrop-blur-sm'} border border-white/20 p-6 flex flex-col h-full`}
            >
              <div className={`mb-4 rounded-lg p-3 inline-flex ${client.highlight ? 'bg-teal text-white' : 'bg-teal/10 text-teal'}`}>
              ...
              {client.highlight && (
                <div className="mt-4 pt-4 border-t border-white/20">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-teal">Featured Client</span>
                  </div>
                </div>
              )}
            </motion.div>
